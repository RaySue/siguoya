...
public:
    ...
    // 纯数字转字符串函数
    int strToNum(bool flag, string str)
    {
        long N = 0;
    ...
    int strToInt1(string str)
    {
        ...
        for (int i = 0; i < str.size(); ++i)
        {
            if (str[i] == ' ' && !numFirst && !signFirst) continue;
            else if ((str[i] == '+' && !signFirst && !numFirst) || (str[i] == '-' && !signFirst && !numFirst))
            {
                signFirst = true;
                if (str[i] == '-') flag = false;
