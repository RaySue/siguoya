...
public:
    ...
    // 思路：
    // 申请 status 表示参议院每个人的状态
    // 遍历参议院
    // 如果是 R 那么就把后面的 D 干掉，注意如果后面没有D了，那么就把前面的 D 干掉（这里使用遍历两圈的方式）
    // 同理，如果是 D 就把后面的 R 干掉，注意如果后面没有 R了，那么就把前面的 R 干掉
    // Complexity:
    // O((2N)^2)
    // O(1)
    string predictPartyVictory1(string senate)
    {
        if (senate.empty()) return "Dire";
        vector<int> status(senate.size(), 1);

        while (true)
        {
            for (int i = 0; i < senate.size(); ++i)
            {
                if (senate[i] == 'R' && status[i] != 0)
                {
                    int flag = true;
                    for (int j = i + 1; j < 2 * senate.size(); ++j)
                    {
                        if (senate[j % senate.size()] == 'D' && status[j % senate.size()] != 0)
                        {
                            flag = false;
                            status[j % senate.size()]--;
                            break;
                        }
                    }
                    if (flag) return "Radiant";
                } else if (senate[i] == 'D' && status[i] != 0)
                {
                    int flag = true;
                    for (int j = i + 1; j < 2 * senate.size(); ++j)
                    {
                        if (senate[j % senate.size()] == 'R' && status[j % senate.size()] != 0)
                        {
                            flag = false;
                            status[j % senate.size()]--;
                            break;
