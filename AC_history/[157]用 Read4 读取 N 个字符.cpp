...
public:
    /**
     *
     * 这个题的一个考点在于，理解和使用read4这个函数
     * read4输入是 char[] 而返回的是读取的长度，每次读取的长度是 小于等于4的
     * 循环使用read4，读取n个字符，根据实际字符的长度 length 有两种情况
     *
     * char a[4];
     * int len = read4(a);
     * 返回实际读取的长度 len，和读取的结果字符数组 a
     *
     * 1. length < n，那么读取 length 个字符并返回 length
     * 2. length >= n，那么读取 n 个字符并返回 n
     *
     *
     *
     *
    ...
    int read(char *buf, int n)
    {
        ...
        int length = 0;
        int tmp;
        while((tmp = read4(a)) > 0 && length < n)  // length >= n 的时候停止
        {
            for (int i = 0; i < tmp; ++i)
            {
                buf[length++] = a[i];
                if (length >= n) break;  // length >= n 的时候就停止
            }
        }
        return length;
    }
